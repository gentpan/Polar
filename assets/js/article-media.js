(() => {
 ...
 function mount(){
  ...
  const preview=document.createElement('div');preview.className='feng-link-preview';preview.hidden=true;preview.setAttribute('role','tooltip');preview.id='feng-link-preview';
  const shot=document.createElement('img'),caption=document.createElement('div');shot.alt='网站预览';preview.append(shot,caption);document.body.append(preview);
  let previewTimer,activeLink;
  const hidePreview=()=>{clearTimeout(previewTimer);preview.hidden=true;if(activeLink){const ids=(activeLink.getAttribute('aria-describedby')||'').split(' ').filter(id=>id&&id!==preview.id);if(ids.length)activeLink.setAttribute('aria-describedby',ids.join(' '));else activeLink.removeAttribute('aria-describedby');}activeLink=null;};
  const showPreview=link=>{hidePreview();previewTimer=setTimeout(()=>{const url=new URL(link.href);activeLink=link;caption.textContent=url.hostname;shot.hidden=false;shot.onerror=()=>{shot.hidden=true;};shot.src='https://s.wordpress.com/mshots/v1/'+encodeURIComponent(url.href)+'?w=640&h=400';preview.hidden=false;link.setAttribute('aria-describedby',((link.getAttribute('aria-describedby')||'')+' '+preview.id).trim());const r=link.getBoundingClientRect(),w=preview.offsetWidth,h=preview.offsetHeight;preview.style.left=Math.max(12,Math.min(innerWidth-w-12,r.left+r.width/2-w/2))+'px';preview.style.top=(r.top-h-12>=12?r.top-h-12:Math.min(innerHeight-h-12,r.bottom+12))+'px';},350);};
  document.querySelectorAll('.xf-prose a.feng-external-link').forEach(link=>{
   link.addEventListener('mouseenter',()=>{if(matchMedia('(hover:hover)').matches)showPreview(link);},{signal:life.signal});
   link.addEventListener('mouseleave',hidePreview,{signal:life.signal});
   link.addEventListener('focus',()=>{if(link.matches(':focus-visible'))showPreview(link);},{signal:life.signal});
   link.addEventListener('blur',hidePreview,{signal:life.signal});
   link.addEventListener('click',hidePreview,{signal:life.signal});
  });
  document.addEventListener('keydown',e=>{if(e.key==='Escape')hidePreview();},{signal:life.signal});
  window.addEventListener('scroll',hidePreview,{passive:true,signal:life.signal});window.addEventListener('resize',hidePreview,{signal:life.signal});
  ...
  cleanup=()=>{hidePreview();preview.remove();life.abort();observer.disconnect();window.LiteZoom?.close();};
 }

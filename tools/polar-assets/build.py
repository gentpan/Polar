...
def css(text,url):
 def rewrite(match):
  ...
  if '/wp-content/themes/polar/assets/' in resolved:resolved='../'+resolved.split('/wp-content/themes/polar/assets/',1)[1]
  return 'url("'+resolved+'")'
...
def stylesheet(handle,source):
 ...
 else:path=src/source;url='http://xifeng.local/wp-content/themes/polar/'+source

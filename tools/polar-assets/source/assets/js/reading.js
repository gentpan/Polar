...
(() => {
 ...
 const mount=()=>{
  ...
  if(!links.length){toc.querySelector('summary span').textContent='阅读进度';nav.innerHTML='<p>本文没有分节标题，可以直接阅读。</p>';}
  nav.addEventListener('click',e=>{if(e.target.closest('a'))toc.open=false;},options);
  document.addEventListener('pointerdown',e=>{if(!toc.contains(e.target))toc.open=false;},options);
  document.addEventListener('keydown',e=>{if(e.key==='Escape'&&toc.open){toc.open=false;toc.querySelector('summary').focus();}},options);
  const update=()=>{
   frame=0;const rect=text.getBoundingClientRect(),offset=focus?32:(parseFloat(getComputedStyle(document.documentElement).scrollPaddingTop)||120)+20;
   const percent=Math.round(Math.min(100,Math.max(0,(offset-rect.top)/Math.max(1,rect.height-innerHeight+offset+40)*100)));
   tools.querySelector('progress').value=percent;tools.querySelector('[data-reading-percent]').textContent=percent+'%';
  ...
  tools.querySelector('[data-reading-focus]').addEventListener('click',e=>{
   const button=e.currentTarget;focus=!focus;toc.open=false;if(focus)focusScroll=window.scrollY;
  ...
  tools.querySelector('[data-reading-print]').addEventListener('click',()=>{toc.open=false;window.print();},options);

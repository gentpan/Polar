...
<?php foreach($links as $friend):
 $snapshot=get_option('feng_feed_source_'.(int)$friend->link_id,array());
 if(($snapshot['hash']??'')!==hash('sha256',trim($friend->link_rss)))$snapshot=array();
 $latest_update=0;$now=time();foreach($snapshot['items']??array() as $item){$published=(int)($item['published']??0);if($published<=$now)$latest_update=max($latest_update,$published);}
 $update_level=$latest_update>=$now-3*DAY_IN_SECONDS?'fresh':($latest_update>=$now-7*DAY_IN_SECONDS?'week':'');
 $update_label=$update_level==='fresh'?'最近三天有新文章':($update_level==='week'?'最近一周有新文章':'');
...
<?php
$friend_info=array(
 '博主名称'=>feng_setting('profile_name','')?:get_bloginfo('name'),
...
<dl><?php foreach($friend_info as $label=>$value): ?><div><dt><?php echo esc_html($label); ?></dt><dd><?php echo esc_html($value?:'暂未设置'); ?></dd><?php if($value): ?><button type="button" data-friend-copy="<?php echo esc_attr($value); ?>" aria-label="<?php echo esc_attr('复制'.$label); ?>" title="<?php echo esc_attr('复制'.$label); ?>"><i class="fa-regular fa-copy" aria-hidden="true"></i></button><?php endif; ?></div><?php endforeach; ?></dl></dialog>
